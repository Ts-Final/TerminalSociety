...
import {Status, status} from "../../../../core/functions/status.ts";
...
<template>
  <div class="gameUnit medium-size flex-col style-border">
    ...
    <div class="flex-row">
      <input class="style-border center-text" v-model="amount" placeholder="数量">
      <div class="flex-col center-text" style="flex-grow: 1">
        <div :class="{'btn-disabled':!Status.toBoolean(canBuy),'btn':Status.toBoolean(canBuy)}"
             @click="doBuyExchange(index,amount, Status.toBoolean(canBuy))">购买
        </div>
        <div :class="{'btn-disabled':!canSell,'btn':canSell}" class="border1-top"
             @click="doSellExchange(index,amount, Status.toBoolean(canSell))">卖出
        </div>
      </div>
    </div>
